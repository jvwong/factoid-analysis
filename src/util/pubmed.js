...
import { NCBI_EUTILS_BASE_URL, NCBI_EUTILS_API_KEY } from '../config.js';
...
const getMedlineCitation = PubmedArticle => {
  ...
  const PMID_elt = getElementByName( MedlineCitation, 'PMID' );
  const PMID = getElementText( PMID_elt );
...
const eFetchPubmed = ( { uids, query_key, webenv, opts } )=> {
  ...
  const userAgent = `factoid-analysis/0.1.0`;
